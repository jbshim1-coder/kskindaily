...
interface Post {
  ...
  content_en: string; content_zh: string; content_ja: string;
  content_vi: string; content_th: string; content_ru: string; content_mn: string;
...
function getTitle(post: Post | RelatedPost, lang: Locale): string {
  const key = `title_${lang}` as keyof typeof post;
  return (post[key] as string) || post.title_en;
}

function getContent(post: Post, lang: Locale): string {
  const key = `content_${lang}` as keyof Post;
...
async function getPost(slug: string): Promise<Post | null> {
  const { data } = await supabase
    .from('blog_posts')
    .select('slug, title_en, title_zh, title_ja, title_vi, title_th, title_ru, title_mn, content_en, content_zh, content_ja, content_vi, content_th, content_ru, content_mn, excerpt_en, image_url, image_alt, category, hashtags, published_at, updated_at')
    .eq('slug', slug)
...
async function getRelatedPosts(category: string, currentSlug: string): Promise<RelatedPost[]> {
  const { data } = await supabase
    .from('blog_posts')
    .select('slug, title_en, title_zh, title_ja, title_vi, title_th, title_ru, title_mn, image_url, published_at')
    .eq('is_published', true)
    .eq('site', siteId)
  ...
  return {
    ...
    alternates: {
      languages: Object.fromEntries(
        LOCALES.map(l => [l, `${siteDomain}/${l}/${slug}`])
      ),
    },
  };
}
...
export default async function LocalePostPage({
  ...
  const articleSchema = {
    ...
    '@type': 'Article',
    headline: title,
    description: post.excerpt_en || title,
    ...
    <>
      ...
      <article className="mx-auto max-w-2xl px-4 py-10">
        <nav className="flex items-center gap-1.5 text-xs text-gray-400 mb-6">
          <Link href={`/${lang}`} className="hover:text-gray-600">Home</Link>
          <span>/</span>
        ...
        <div className="flex gap-1 flex-wrap mb-6">
          {LOCALES.map((l) => (
            <Link
              ...
              style={l === lang ? { backgroundColor: siteColor } : {}}
            >
              {LANG_LABELS[l]}
            </Link>
          ))}
            ...
            <div className="grid grid-cols-2 gap-3">
              {related.map((rp) => (
                ...
                >
                  ...
                  <p className="text-xs font-medium text-gray-700 line-clamp-2">{getTitle(rp, lang)}</p>
                </Link>
              ))}
            </div>
          </div>
